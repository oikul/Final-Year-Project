...
public class VoronoiGenerator {
	...
	public VMesh generateVoronoi(int numOfPoints, float minX, float maxX, float minZ, float maxZ, float yHeight,
			float range) {
	...
	public BufferedImage createVoronoiImage(int numOfPoints, float minX, float maxX, float minZ, float maxZ,
			float range) {
		double[] xValuesIn = new double[numOfPoints], zValuesIn = new double[numOfPoints];
		for (int i = 0; i < numOfPoints; i++) {
		...
		if (numOfPoints > 0) {
			...
			for (int i = 0; i < graphEdges.size(); i++) {
				...
				g.drawLine((int) graphEdges.get(i).x1, (int) graphEdges.get(i).y1, (int) graphEdges.get(i).x2,
						(int) graphEdges.get(i).y2);
			}
		}
		// try {
