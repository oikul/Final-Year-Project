...
public class Terrain {
	...
	public Terrain(int blocksPerRow, float scale, int width, int height, int textInc, long seed, float amplitude, float roughness, int octaves, int poctave1, int poctave2, boolean perlinorvalue, int voronoiSize) {
		chunks = new Entity[blocksPerRow * blocksPerRow];
		HeightMapMesh mesh = new HeightMapMesh(width, height, textInc, seed, amplitude, roughness, octaves, poctave1, poctave2, perlinorvalue, voronoiSize);
